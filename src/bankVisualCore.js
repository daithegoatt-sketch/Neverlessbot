...
function helpCard() {
  ...
  sections.forEach(([title, commands], index) => {
    ...
    rtlText(ctx, title, x + w - 24, y + 38, '800 21px sans-serif', THEME.silver);
    commands.forEach((command, i) => {
      ...
      ctx.fill();
      rtlText(ctx, command, x + w - 42, yy, '600 16px sans-serif', THEME.text);
    });
  });

  rtlText(ctx, 'خيارات المبلغ: كامل • نص • ربع • أو رقم محدد', 1042, 812, '600 14px sans-serif', THEME.muted);
...
async function balanceCard(user, state, price) {
  ...
  rtlText(ctx, `سعر سهم Neverless: ${money(price)}`, 1025, 530, '600 14px sans-serif', THEME.muted);
  return canvas.toBuffer('image/png');
}
...
async function rewardCard(user, label, amount, balance, kind) {
  ...
  ctx.fillStyle = THEME.text;
  ctx.font = `800 ${fitText(ctx, playerName(user), 250, 26, 17, 800)}px sans-serif`;
  ...
  centerText(ctx, kind === 'salary' ? 'تم إيداع الراتب' : 'وصلتك مكافأة', 715, 190, '700 19px sans-serif', THEME.muted);
  ctx.textAlign = 'center';
  ctx.fillStyle = accent;
  ctx.font = '900 55px sans-serif';
  ctx.fillText(`+${money(amount)}`, 715, 260);
  ctx.textAlign = 'left';
  centerText(ctx, `الرصيد الآن ${money(balance)}`, 715, 307, '700 17px sans-serif', THEME.text);
  rtlText(ctx, 'متاح مجدداً بعد 5 دقائق', 910, 397, '600 15px sans-serif', THEME.muted);
  return canvas.toBuffer('image/png');
}
...
async function vaultCard(user, action, amount, state) {
  ...
  ctx.fillStyle = THEME.text;
  ctx.font = `800 ${fitText(ctx, playerName(user), 240, 24, 16, 800)}px sans-serif`;
  ...
  rtlText(ctx, deposit ? 'تم تأمين المبلغ في خزنتك' : 'تم تحويل المبلغ إلى رصيدك المتاح', 910, 397, '600 15px sans-serif', THEME.muted);
  return canvas.toBuffer('image/png');
}
...
async function transferCard(from, to, amount, fromBalance, toBalance) {
  ...
  ctx.fillStyle = THEME.text;
  ctx.font = `800 ${fitText(ctx, playerName(from), 245, 23, 15, 800)}px sans-serif`;
  ctx.fillText(playerName(from), 70, 335);
  ctx.textAlign = 'right';
  ctx.font = `800 ${fitText(ctx, playerName(to), 245, 23, 15, 800)}px sans-serif`;
  ...
  centerText(ctx, 'قيمة التحويل', 550, 190, '600 16px sans-serif', THEME.muted);
  ctx.textAlign = 'center';
  ctx.fillStyle = THEME.green;
...
function marketCard(market) {
  ...
  rtlText(ctx, 'يتحدث السعر كل 5 دقائق', 978, 204, '600 15px sans-serif', THEME.muted);

  fillRoundRect(ctx, 55, 260, 940, 250, 22, 'rgba(7,17,30,.93)', THEME.strokeSoft, 1.5);
  drawLineChart(ctx, history, 92, 300, 866, 165, color);
  rtlText(ctx, 'شراء سهم 5 • بيع سهم 5 • يدعم كامل / نص / ربع', 990, 552, '600 14px sans-serif', THEME.muted);
  return canvas.toBuffer('image/png');
}
...
async function topCard(rows, price) {
  ...
  for (let i = 0; i < Math.min(5, rows.length); i += 1) {
    ...
    ctx.fillStyle = THEME.text;
    ctx.font = `800 ${fitText(ctx, playerName(row.user), 350, 22, 15, 800)}px sans-serif`;
...
function statusCard(user, state) {
  ...
  ctx.fillStyle = THEME.text;
  ctx.font = `800 ${fitText(ctx, playerName(user), 330, 25, 16, 800)}px sans-serif`;
  ...
  entries.forEach(([label, ms], index) => {
    ...
    rtlText(ctx, label, x + 445, y + 41, '800 18px sans-serif', THEME.text);
    const good = ms <= 0;
...
function infoCard(title, message, ok = false) {
  ...
  centerText(ctx, message, 450, 205, '800 21px sans-serif', THEME.text);
  return canvas.toBuffer('image/png');
}
