...
const USER_PREFIX = 'NLHID2|';
const GLOBAL_PREFIX = 'NLHIDG2|';
const LOCAL_FILE = path.join(DATA_DIR, 'hidden-achievements-v2.json');
...
function normalizeProgress(row = {}) {
  return {
    ...
    messageId: /^\d{15,22}$/.test(String(row.messageId || '')) ? String(row.messageId) : null,
  };
}

function normalizeUserRecord(input = {}) {
  const achievements = {};
  for (const [id, row] of Object.entries(input.achievements || {})) if (ACHIEVEMENTS[id]) achievements[id] = normalizeProgress(row);
  return {
...
function compactUser(record) {
  const h = {};
  for (const [id, row] of Object.entries(record.achievements || {})) h[id] = [row.unlockedAt || null, row.claimedAt || null, row.messageId || null];
  return { d: record.lastActiveDay || '', s: cleanInt(record.currentStreak), b: cleanInt(record.bestStreak), a: cleanInt(record.activeDays), v: cleanInt(record.voiceMs), r: record.roomId || null, h, u: record.updatedAt || null };
}

function expandUser(input = {}) {
  if (!Object.prototype.hasOwnProperty.call(input, 'h')) return normalizeUserRecord(input);
  const achievements = {};
  for (const [id, row] of Object.entries(input.h || {})) {
    if (!ACHIEVEMENTS[id] || !Array.isArray(row)) continue;
    achievements[id] = { unlockedAt: row[0] || null, claimedAt: row[1] || null, messageId: row[2] || null };
  }
  return normalizeUserRecord({ lastActiveDay: input.d, currentStreak: input.s, bestStreak: input.b, activeDays: input.a, voiceMs: input.v, roomId: input.r, achievements, updatedAt: input.u });
}

function compactGlobal(global) {
  return { l: global.launchedAt || null, a: global.announcedAt || null, w: global.raceWinnerId || null, t: global.raceWinnerAt || null, u: global.updatedAt || null };
}

function expandGlobal(input = {}) {
  if (Object.prototype.hasOwnProperty.call(input, 'l') || Object.prototype.hasOwnProperty.call(input, 'w')) return normalizeGlobal({ launchedAt: input.l, announcedAt: input.a, raceWinnerId: input.w, raceWinnerAt: input.t, updatedAt: input.u });
  return normalizeGlobal(input);
}
...
function parseUserMessage(content) {
  ...
  return decoded ? { guildId, userId, record: expandUser(decoded) } : null;
}
...
function parseGlobalMessage(content) {
  ...
  const decoded = decode(rest.slice(split + 1));
  return /^\d{15,22}$/.test(guildId) && decoded ? { guildId, global: expandGlobal(decoded) } : null;
}
...
function ensureLocalLoaded() {
  ...
  try {
    ...
    state = { users: parsed.users || {}, global: expandGlobal(parsed.global || {}) };
    for (const [id, record] of Object.entries(state.users)) state.users[id] = expandUser(record);
  } catch (error) {
...
async function writeLocal() {
  const users = {};
  for (const [id, record] of Object.entries(state.users)) users[id] = compactUser(record);
  await fs.promises.mkdir(DATA_DIR, { recursive: true });
  await fs.promises.writeFile(LOCAL_FILE, JSON.stringify({ users, global: compactGlobal(state.global) }, null, 2));
}
...
function persistUser(guild, userId) {
  ...
  persistQueue = persistQueue.then(async () => {
    ...
    const content = `${USER_PREFIX}${guild.id}|${id}|${encode(compactUser(record))}`;
    if (content.length > 1900) throw new Error('HIDDEN_USER_RECORD_TOO_LARGE');
...
function persistGlobal(guild) {
  ...
  persistQueue = persistQueue.then(async () => {
    await writeLocal();
    if (!dataChannel || dataChannel.guildId !== guild.id) return;
    const content = `${GLOBAL_PREFIX}${guild.id}|${encode(compactGlobal(state.global))}`;
...
async function loadRemoteState(guild) {
  ...
  for (const [userId, row] of latestUsers) {
    state.users[userId] = row.record;
    userMessageIds.set(userId, row.messageId);
  }
  if (latestGlobal) {
    state.global = latestGlobal.global;
...
function claimRow(achievementId, userId, disabled = false) {
  return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`hidden:claim:${achievementId}:${userId}`).setLabel(disabled ? 'Claimed' : 'Claim Now').setStyle(disabled ? ButtonStyle.Secondary : ButtonStyle.Success).setDisabled(disabled));
}

function unlockEmbed(achievement) {
  return new EmbedBuilder().setColor(0x66578a).setTitle('🏆 Hidden Achievement Unlocked').setDescription([
...
async function ensureClaimPrompt(guild, userId, achievementId) {
  ...
  const message = await room.send({ content: `<@${userId}> مبروك، حصلت على إنجاز مخفي.`, embeds: [unlockEmbed(achievement)], components: [claimRow(achievementId, userId)], allowedMentions: { users: [userId] } });
...
async function unlockAchievement(guild, userId, achievementId) {
  ...
  record.achievements[achievementId] = { unlockedAt: new Date().toISOString(), claimedAt: null, messageId: null };
  await persistUser(guild, userId);
...
async function evaluateUser(guild, userId) {
  const record = userRecord(userId);
  for (const id of standardAchievementIds(record, activityXp.get(String(userId)) || 0)) await unlockAchievement(guild, userId, id);
}
...
async function handleClaim(interaction) {
  ...
  const rewardRole = rewardRoles.get(parsed.achievementId);
  if (!member || !rewardRole || !accessRole) {
    await interaction.followUp({ content: 'تعذر تجهيز الجائزة الآن. جرّب بعد قليل.', ephemeral: true });
    return true;
  }
  try { await member.roles.add([rewardRole.id, accessRole.id], `Claimed hidden achievement: ${ACHIEVEMENTS[parsed.achievementId].name}`); }
  catch {
...
async function handleHiddenCommand(message, command) {
  ...
  const claimed = Object.entries(record.achievements)
    ...
    .map(([id, progress]) => ({ progress, achievement: ACHIEVEMENTS[id] }))
