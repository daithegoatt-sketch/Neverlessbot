...
async function createTicket(interaction, config) {
  if (!config.ticketCategoryId || !config.supportRoleId) {
    return interaction.reply({ content: 'نظام التذاكر غير مهيأ بعد. استخدم /ticket-setup أولاً.', ephemeral: true });
  }
  ...
  await channel.send({ content: `${interaction.user} <@&${config.supportRoleId}>`, embeds: [embed], components: ticketControls() });
...
module.exports = { ticketPanelComponents, createTicket, handleTicketButton, isStaff };
