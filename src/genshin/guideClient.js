...
function normalizeTeams(value) {
  ...
  return {
    premium: Array.isArray(value.premium) ? value.premium : [],
    f2p: Array.isArray(value.f2p) ? value.f2p : [],
  };
}
...
function mergeGuide(live, curated) {
  ...
  const liveTeams = normalizeTeams(live.teams);
  const fallbackTeams = normalizeTeams(curated.teams);
  return {
    ...curated,
    ...live,
    ...
    teams: {
      premium: nonEmpty(liveTeams.premium) ? liveTeams.premium : fallbackTeams.premium,
      f2p: nonEmpty(liveTeams.f2p) ? liveTeams.f2p : fallbackTeams.f2p,
    },
  };
}
...
async function getGuide(name) {
  ...
  let live = null;
  try {
    live = await fetchGame8Guide(name);
  } catch (error) {
