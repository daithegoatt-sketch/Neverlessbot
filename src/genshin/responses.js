...
function buildText(guide, lang, f2p = false) {
  ...
  if (guide.artifacts?.length) {
    lines.push(`**${A ? 'الآرتيفاكت' : 'Artifacts'}:** ${guide.artifacts.slice(0, 3).join(' / ')}`);
  }
  if (guide.stats?.main?.length) {
    lines.push(`**${A ? 'تقسيم القطع' : 'Main Stats'}:**`);
  ...
  if (guide.stats?.targets?.length) {
    lines.push(`**${A ? 'الأرقام اللي تستهدفها' : 'Stat Targets'}:**`);
...
function artifactsText(guide, lang) {
  const A = ar(lang);
  const lines = [`**${guide.name} — ${A ? 'الآرتيفاكت' : 'Artifacts'}**`];
...
function weaponsText(guide, lang, f2p = false) {
  ...
  if (!weapons.length) lines.push(A ? 'ما عندي ترتيب موثوق للأسلحة حاليًا.' : 'No reliable weapon ranking is available right now.');
  else weapons.slice(0, 8).forEach((item, i) => lines.push(`${i + 1}. ${item}`));
  return lines.join('\n');
}

function statsText(guide, lang) {
  const A = ar(lang);
  const lines = [`**${guide.name} — ${A ? 'الإحصائيات المطلوبة' : 'Recommended Stats'}**`];
  if (guide.stats?.targets?.length) guide.stats.targets.slice(0, 8).forEach((item) => lines.push(`• ${item}`));
  else lines.push(A ? 'ما عندي أرقام Target موثقة لهذه الشخصية حاليًا.' : 'No reliable target ranges are available for this character right now.');
...
function teamText(guide, lang, type = 'premium', limit = 4) {
  const A = ar(lang);
  const teams = normalizeTeams(guide, type);
  const typeText = type === 'f2p' ? 'F2P' : (A ? 'أفضل التيمات' : 'Best Teams');
  const lines = [`**${guide.name} — ${typeText}**`];
  if (!teams.length) {
    lines.push(A ? 'ما لقيت تيم منشور بهذا التصنيف حاليًا.' : 'No published team is available for that category right now.');
    return lines.join('\n');
  }
  teams.slice(0, limit).forEach((team, i) => lines.push(`${i + 1}. ${team.join(' • ')}`));
  return lines.join('\n');
}
...
function baseText(character, stats, lang) {
  ...
  return [
    `**${character?.name || 'Character'} — ${A ? 'Base Stats' : 'Base Stats'}**`,
...
function opinionText(guide, lang) {
  const A = ar(lang);
  if (!A) return `**${guide.name}**: ${guide.role || 'I have build and team data for this character.'} Ask for the build, teams, F2P team, weapons, or stats and I’ll give only that part.`;
  const role = guide.role || 'عندي لها بيانات بيلد وتيمات';
  return `**${guide.name}**: ${role}. إذا تبي أقدر أعطيك البيلد، التيمات، تيم F2P، الأسلحة أو الستات المطلوبة — حسب اللي تسأل عنه فقط.`;
}

function closestReplacement(currentTeam, alternatives, missingNames) {
  const missing = new Set(missingNames.map((x) => String(x).toLowerCase()));
  const base = currentTeam || [];
  const candidates = alternatives
    .filter((team) => !team.some((name) => missing.has(String(name).toLowerCase())))
    .map((team) => {
      const same = team.filter((member) => base.some((x) => String(x).toLowerCase() === String(member).toLowerCase())).length;
      const replacements = team.filter((member) => !base.some((x) => String(x).toLowerCase() === String(member).toLowerCase()));
      return { team, same, replacements };
    })
...
function replacementText(character, currentTeam, alternatives, missingNames, lang) {
  const A = ar(lang);
  const best = closestReplacement(currentTeam, alternatives, missingNames);
  if (!best) return A
    ? `ما عندي بديل منشور قريب لنفس تيم **${character}** بعد استبعاد ${missingNames.join(', ')}.`
    : `I don't have a close published replacement for that **${character}** team after excluding ${missingNames.join(', ')}.`;
  const replacement = best.replacements.join(' / ');
  const missing = missingNames.join(', ');
  return A
    ? `إذا ما عندك **${missing}**، أقرب بديل منشور هو **${replacement || 'تغيير التشكيلة'}**.\nالتيم يصير: **${best.team.join(' • ')}**`
    : `If you don't have **${missing}**, the closest published replacement is **${replacement || 'a different shell'}**.\nUse: **${best.team.join(' • ')}**`;
}

function accountEvaluationText(snapshot, evaluation, comparison, guide, lang, akashaPercentile = null) {
  const A = ar(lang);
  const lines = [`**${snapshot.name} — ${A ? 'تقييم بيلدك' : 'Build Rating'}: ${evaluation.score}%**`];
  ...
  if (weak.length) {
    lines.push(`**${A ? 'أهم شيء تحسنه' : 'Main improvements'}:**`);
    weak.forEach((n) => lines.push(`• ${n.text}`));
  } else {
    lines.push(A ? 'الأرقام الأساسية اللي أقدر أقارنها قريبة من الأهداف المنشورة.' : 'The main comparable stats are close to the published targets.');
  }
  ...
  if (comparison) {
    ...
    const useful = Object.entries(comparison.deltas).filter(([, value]) => value !== 0).slice(0, 5);
    if (useful.length) lines.push(useful.map(([key, value]) => `${LABELS[key] || key} ${value > 0 ? '+' : ''}${value}`).join(' • '));
    lines.push(comparison.scoreDelta > 0
      ? (A ? 'البيلد تحسّن عن النسخة السابقة.' : 'The build improved over the previous version.')
      : comparison.scoreDelta < 0
        ? (A ? 'التقييم نزل عن النسخة السابقة؛ راجع الستات اللي نقصت.' : 'The score dropped; check the stats that decreased.')
        : (A ? 'التقييم العام ثابت تقريبًا.' : 'The overall score is essentially unchanged.'));
  }
  return lines.join('\n');
}
