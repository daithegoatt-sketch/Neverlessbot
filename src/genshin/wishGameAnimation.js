...
// Source is pinned so the recordings cannot silently change underneath Neverless.
...
// Edit into the first item before the dark tail of the recording can sit on screen.
// The GIF itself is also encoded as play-once below, so it no longer repeats.
function animationWaitMs(capturingRadiance = false) {
  return capturingRadiance ? 8200 : 6800;
}
...
async function fetchSource(filename) {
  ...
  const promise = (async () => {
    ...
    const response = await fetch(url, {
      headers: { 'user-agent': 'Neverless-Wish/1.1' },
...
async function transcodeGif(inputPath, profile) {
  ...
  const child = spawn(ffmpegPath, [
    ...
    '-map', '[v]',
    // -1 means no Netscape infinite-loop extension: play the recorded wish once.
    '-loop', '-1',
    '-f', 'gif',
    'pipe:1',
  ...
  const promise = (async () => {
    ...
    try {
      ...
      for (const profile of PROFILES) {
        try { return await transcodeGif(inputPath, profile); }
        catch (error) {
          lastError = error;
