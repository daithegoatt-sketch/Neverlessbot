...
const ACHIEVEMENTS = Object.freeze({
  message500: {
    name: 'First Echo',
    role: 'Hidden • First Echo',
    condition: 'وصلت إلى 500 رسالة مؤهلة في نظام نشاط Neverless.',
  },
  message1000: {
    name: 'A Thousand Words',
    role: 'Hidden • Thousand Words',
    condition: 'وصلت إلى 1,000 رسالة مؤهلة في نظام نشاط Neverless.',
  },
  streak7: {
    name: 'Still Here',
    role: 'Hidden • Still Here',
    condition: 'حافظت على نشاط مؤهل 7 أيام متتالية.',
  },
  streak14: {
    name: 'Unbroken',
    role: 'Hidden • Unbroken',
    condition: 'حافظت على نشاط مؤهل 14 يومًا متتاليًا.',
  },
  streak30: {
    name: 'Never Fades',
    role: 'Hidden • Never Fades',
    condition: 'حافظت على نشاط مؤهل 30 يومًا متتاليًا.',
  },
  voice400: {
    name: 'Voice in the Dark',
    role: 'Hidden • Voice in the Dark',
    condition: 'جمعت 400 دقيقة Voice مؤهلة مع أعضاء آخرين.',
  },
  voice1200: {
    name: 'After Hours',
    role: 'Hidden • After Hours',
    condition: 'جمعت 1,200 دقيقة Voice مؤهلة مع أعضاء آخرين.',
  },
  active45: {
    name: 'The Regular',
    role: 'Hidden • The Regular',
    condition: 'كنت نشطًا في 45 يومًا مختلفًا منذ تفعيل النظام.',
  },
  firstLevel20: {
    name: 'First Ascension',
    role: 'Hidden • First Ascension',
    condition: `كنت أول عضو يصل إلى Lv.${FIRST_LEVEL} بعد بدء سباق الإنجاز المخفي.`,
  },
});
...
function xpForLevel(level) {
  const safe = Math.max(0, cleanInt(level));
...
function userRecord(userId) {
  ...
  if (!state.users[id]) state.users[id] = normalizeUserRecord();
  else state.users[id] = normalizeUserRecord(state.users[id]);
...
function parseGlobalMessage(content) {
  ...
  return global ? { guildId, global: normalizeGlobal(global) } : null;
}
...
function parseActivityXp(content) {
  ...
  if (!/^\d{15,22}$/.test(guildId || '') || !/^\d{15,22}$/.test(userId || '')) return null;
  return { guildId, userId, allXp: cleanInt(parts[3]) };
}
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      state = {
        ...
        global: normalizeGlobal(parsed.global || {}),
      };
      for (const [id, record] of Object.entries(state.users)) state.users[id] = normalizeUserRecord(record);
    }
  } catch (error) {
    console.warn('[hidden-achievements] Local state load failed:', error.message);
  }
}
...
function userContent(guildId, userId, record) {
  return `${USER_PREFIX}${guildId}|${userId}|${encode(record)}`;
}

function globalContent(guildId) {
  return `${GLOBAL_PREFIX}${guildId}|${encode(state.global)}`;
}
...
function persistUser(guild, userId) {
  ...
  persistQueue = persistQueue.then(async () => {
    ...
    const content = userContent(guild.id, id, record);
...
function persistGlobal(guild) {
  ...
  persistQueue = persistQueue.then(async () => {
    await writeLocal();
    if (!dataChannel || dataChannel.guildId !== guild.id) return;
    const content = globalContent(guild.id);
...
async function waitForDataChannel(guild) {
  for (let attempt = 0; attempt < 12; attempt += 1) {
    const cached = guild.channels.cache.find((channel) => channel.name === DATA_CHANNEL_NAME && channel.isTextBased?.());
...
async function loadRemoteState(guild) {
  ...
  if (!dataChannel) {
    console.warn('[hidden-achievements] neverless-data unavailable; local fallback only.');
    return;
  }
  ...
  for (const message of messages) {
    ...
    if (hidden?.guildId === guild.id) {
      ...
      if (!previous || message.createdTimestamp > previous.createdTimestamp) {
        latestUsers.set(hidden.userId, { ...hidden, messageId: message.id, createdTimestamp: message.createdTimestamp });
      }
    }

    const global = parseGlobalMessage(message.content);
    if (global?.guildId === guild.id && (!latestGlobal || message.createdTimestamp > latestGlobal.createdTimestamp)) {
      latestGlobal = { ...global, messageId: message.id, createdTimestamp: message.createdTimestamp };
    }

    const activity = parseActivityXp(message.content);
    if (activity?.guildId === guild.id) {
      activityXp.set(activity.userId, Math.max(activityXp.get(activity.userId) || 0, activity.allXp));
    }
  }
...
async function ensureRole(guild, name) {
  ...
  if (!role) {
    role = await guild.roles.create({
      ...
      hoist: false,
      mentionable: false,
      reason: 'Neverless hidden achievement reward',
    });
  }
  return role;
}
...
async function ensureRewardStructure(guild) {
  ...
  for (const [id, achievement] of Object.entries(ACHIEVEMENTS)) {
    rewardRoles.set(id, await ensureRole(guild, achievement.role));
  }
  ...
  lounge = guild.channels.cache.find((channel) => channel.parentId === category.id && channel.name === LOUNGE_NAME) || null;
  if (!lounge) {
    ...
    await lounge.send({
      embeds: [new EmbedBuilder()
        .setColor(0x4f456b)
        .setTitle('Neverless Secret')
        .setDescription('هذا الروم يفتح تلقائيًا بعد Claim لأول Hidden Achievement. ما فيه صلاحيات إدارية مرتبطة بالرتبة.')],
    }).catch(() => {});
  }
}
...
async function ensureClaimRoom(guild, userId) {
  ...
  if (!room) {
    room = guild.channels.cache.find((channel) => channel.topic?.includes(`neverless-hidden-owner:${userId}`)) || null;
  }
  if (room) {
    if (record.roomId !== room.id) {
...
function unlockEmbed(achievement) {
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      '',
      `اضغط **Claim Now** للحصول على رتبة **${achievement.role}**. أول Claim يفتح أيضًا **Neverless Secret** والـSecret Lounge وأوامر Premium الخاصة بالأعضاء الحاصلين على إنجاز مخفي.`,
    ].join('\n'))
...
async function ensureClaimPrompt(guild, userId, achievementId) {
  ...
  if (!progress?.unlockedAt || progress.claimedAt) return;

  const room = await ensureClaimRoom(guild, userId);
  if (!room) return;

  if (progress.messageId) {
    const existing = await room.messages.fetch(progress.messageId).catch(() => null);
    if (existing) return;
  }
...
async function unlockAchievement(guild, userId, achievementId) {
  ...
  const record = userRecord(userId);
  const current = record.achievements[achievementId];
  if (current?.unlockedAt) {
    if (!current.claimedAt) await ensureClaimPrompt(guild, userId, achievementId);
    return false;
  }
...
async function evaluateUser(guild, userId) {
  const record = userRecord(userId);
  const xp = activityXp.get(String(userId)) || 0;
  for (const achievementId of standardAchievementIds(record, xp)) {
    await unlockAchievement(guild, userId, achievementId);
  }
}

function advanceStreak(record, day) {
  const previous = record.lastActiveDay;
  if (previous === day) return false;
  const previousNumber = dayNumber(previous);
  const currentNumber = dayNumber(day);
  record.currentStreak = previousNumber != null && currentNumber != null && currentNumber - previousNumber === 1
    ? cleanInt(record.currentStreak) + 1
    : 1;
...
function queueUser(userId, task) {
  ...
  const queued = current.finally(() => {
    if (userQueues.get(id) === queued) userQueues.delete(id);
  });
...
function updateActivityXp(guild, userId, nextXp, allowRace = true) {
  ...
  if (
    allowRace
    && !state.global.raceWinnerId
    && previous < xpForLevel(FIRST_LEVEL)
    && next >= xpForLevel(FIRST_LEVEL)
  ) {
    state.global.raceWinnerId = id;
    state.global.raceWinnerAt = new Date().toISOString();
    persistGlobal(guild).catch(() => {});
    queueUser(id, async () => {
      await unlockAchievement(guild, id, 'firstLevel20');
      await evaluateUser(guild, id);
    }).catch(() => {});
  } else if (next > previous) {
    queueUser(id, () => evaluateUser(guild, id)).catch(() => {});
  }
}
...
async function pollActivityFile(guild) {
  let parsed;
  try {
    parsed = JSON.parse(await fs.promises.readFile(path.join(DATA_DIR, 'activity.json'), 'utf8'));
  } catch {
    return;
  }
  const users = parsed?.guilds?.[guild.id]?.users || {};
  for (const [userId, record] of Object.entries(users)) {
    updateActivityXp(guild, userId, cleanInt(record?.allXp), true);
  }
}
...
async function reconcileVoice(guild) {
  ...
  for (const [userId, channelId] of active) {
    ...
    if (!voiceSessions.has(key)) {
      voiceSessions.set(key, { guildId: guild.id, userId, channelId, startedAt: now });
    }
  }
}
...
async function flushVoice(guild) {
  ...
  for (const session of voiceSessions.values()) {
    if (session.guildId !== guild.id) continue;
    await queueUser(session.userId, () => accumulateVoiceSession(session, now));
  }
}
...
async function restoreMemberRewards(guild, userId) {
  ...
  const record = userRecord(userId);
  const claimed = Object.entries(record.achievements).filter(([, row]) => row?.claimedAt);
  ...
  for (const [achievementId] of claimed) {
    const role = rewardRoles.get(achievementId);
...
function parseClaimCustomId(customId) {
  ...
  if (!match || !ACHIEVEMENTS[match[1]]) return null;
  return { achievementId: match[1], userId: match[2] };
}
...
async function handleClaim(interaction) {
  ...
  const rewardRole = rewardRoles.get(parsed.achievementId) || await ensureRole(interaction.guild, ACHIEVEMENTS[parsed.achievementId].role);
  if (!member || !rewardRole || !accessRole) {
    await interaction.reply({ content: 'تعذر تجهيز الجائزة الآن. جرّب بعد قليل.', ephemeral: true });
    return true;
  }
  ...
  try {
    ...
    await interaction.reply({ content: 'البوت ما قدر يعطي الرتبة بسبب ترتيب الرتب أو الصلاحيات.', ephemeral: true });
    return true;
  }

  progress.claimedAt = new Date().toISOString();
  await persistUser(interaction.guild, parsed.userId);
  await interaction.update({ components: [claimRow(parsed.achievementId, parsed.userId, true)] });
  await interaction.followUp({
    content: `تم استلام **${ACHIEVEMENTS[parsed.achievementId].name}**. حصلت على **${rewardRole.name}** وفتح لك **Neverless Secret** والـSecret Lounge وأوامر Premium.`,
    ephemeral: true,
  });
  return true;
}
...
function parseHiddenCommand(text) {
  ...
  if (/^-(?:hidden|انجازات\s+مخفيه|انجازات\s+مخفية|الانجازات\s+المخفيه|الانجازات\s+المخفية)$/iu.test(value)) return { type: 'about' };
  if (/^-hidden\s+achievements$/iu.test(value)) return { type: 'about' };
...
async function handleHiddenCommand(message, command) {
  if (command.type === 'about') {
    ...
    await message.reply({
      embeds: [new EmbedBuilder()
        .setColor(0x4f456b)
        .setTitle('Hidden Achievements')
        .setDescription(`داخل Neverless توجد إنجازات مخفية. شروطها وعددها غير معلنين. بعض الإنجازات مرتبطة بالاستمرارية والنشاط والمشاركة داخل المجتمع. إذا حققت واحدًا، البوت يفتح لك Claim خاص تلقائيًا.${extra}`)],
  ...
  if (command.type === 'stats') {
    ...
    await message.reply({
      embeds: [new EmbedBuilder()
        .setColor(0x4f456b)
        .setTitle('Neverless Secret — Your Stats')
        .setDescription(lines.join('\n'))
        .setFooter({ text: 'يعرض أرقامك فقط؛ شروط الإنجازات التي لم تكتشفها تبقى مخفية.' })],
      allowedMentions: { repliedUser: false },
    });
    return true;
  }
  ...
  const claimed = Object.entries(record.achievements)
    .filter(([, progress]) => progress?.claimedAt && ACHIEVEMENTS[arguments[1]])
  ...
  const lines = claimed.length
    ? claimed.map((row) => `🏆 **${row.achievement.name}** — <t:${Math.floor(Date.parse(row.progress.claimedAt) / 1000)}:d>`)
    : ['ما عندك إنجاز Claimed مسجل حاليًا.'];
  await message.reply({
    embeds: [new EmbedBuilder()
      .setColor(0x4f456b)
      .setTitle('Neverless Secret — Hidden Card')
      .setDescription(lines.join('\n').slice(0, 3900))],
    allowedMentions: { repliedUser: false },
  });
  return true;
...
async function announceSystem(guild) {
  ...
  await channel.send({
    embeds: [new EmbedBuilder()
      .setColor(0x4f456b)
      .setTitle('🏆 Hidden Achievements')
      .setDescription('تم تفعيل إنجازات مخفية داخل Neverless. شروطها وعددها غير معلنين. بعضها يعتمد على الاستمرارية والنشاط والمشاركة. إذا حققت واحدًا، Neverless راح يفتح لك مكافأتك الخاصة تلقائيًا.')],
  });
...
async function restoreAndEvaluate(guild) {
  for (const [userId, record] of Object.entries(state.users)) {
    ...
    for (const [achievementId, progress] of Object.entries(record.achievements || {})) {
      if (ACHIEVEMENTS[achievementId] && progress?.unlockedAt && !progress.claimedAt) {
        await ensureClaimPrompt(guild, userId, achievementId);
      }
    }
  }
...
async function initialize(client) {
  ...
  if (!guild) {
    console.warn(`[hidden-achievements] General chat ${GENERAL_CHAT_ID} not found.`);
    return;
  }
  targetGuild = guild;
  ...
  state.global = normalizeGlobal(state.global);
  if (!state.global.launchedAt) {
    state.global.launchedAt = new Date().toISOString();
    await persistGlobal(guild);
  }

  // Remote NLACT1 data is the startup baseline. Existing members already above Lv.20
  // do not retroactively win First Ascension; only a future crossing can win it.
...
function installHiddenAchievements(client) {
  ...
  client.once('ready', () => {
    initialize(client).catch((error) => console.error('[hidden-achievements] Initialization failed:', error));
  });
  ...
  client.on('messageCreate', (message) => {
    ...
    if (command) {
      handleHiddenCommand(message, command).catch((error) => console.error('[hidden-achievements] Command failed:', error));
      return;
    }
  ...
  client.on('interactionCreate', (interaction) => {
    handleClaim(interaction).catch((error) => console.error('[hidden-achievements] Claim failed:', error));
  });

  client.on('voiceStateUpdate', (oldState, newState) => {
    const guild = newState.guild || oldState.guild;
    if (!targetGuild || guild?.id !== targetGuild.id) return;
    reconcileVoice(guild).catch((error) => console.warn('[hidden-achievements] Voice reconcile failed:', error.message));
  });
  ...
  client.on('guildMemberAdd', (member) => {
    ...
    queueUser(member.id, async () => {
      ...
      if (!record) return;
      for (const [id, progress] of Object.entries(record.achievements || {})) {
        if (ACHIEVEMENTS[id] && progress?.unlockedAt && !progress.claimedAt) await ensureClaimPrompt(member.guild, member.id, id);
      }
    }).catch(() => {});
  });
  ...
  setInterval(() => {
    ...
    pollActivityFile(targetGuild).catch((error) => console.warn('[hidden-achievements] Activity poll failed:', error.message));
  }, ACTIVITY_POLL_MS).unref?.();

  setInterval(() => {
    if (!targetGuild) return;
    flushVoice(targetGuild).catch((error) => console.warn('[hidden-achievements] Voice flush failed:', error.message));
  }, VOICE_FLUSH_MS).unref?.();
}
