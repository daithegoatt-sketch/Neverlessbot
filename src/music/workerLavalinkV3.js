...
function scheduleEmptyLeave(guild) {
  ...
  // A pinned worker stays in its pinned VC even when the room is empty.
...
async function reconnectPinned(guild) {
  const voiceId = state.pinnedVoiceId;
  if (!voiceId || !backendUsable()) return;
  ...
  if (String(actualVoiceId(guild) || '') === String(voiceId)) return;
  await ensurePlayer(guild, voiceId, null).catch((error) => {
    console.warn(`[music-worker ${WORKER_ID}] Could not restore pinned VC:`, error?.message || error);
  });
}
...
async function bootstrapGuild(guild) {
  ...
  if (batch) {
    ...
    if (pin) {
      ...
      const wanted = parsed?.payload?.voice_id ? String(parsed.payload.voice_id) : null;
      const voice = wanted ? guild.channels.cache.get(wanted) || await guild.channels.fetch(wanted).catch(() => null) : null;
      state.pinnedVoiceId = voice?.isVoiceBased?.() ? wanted : null;
    }
  }
  await publishStatus(guild);
  if (state.pinnedVoiceId) await reconnectPinned(guild).catch(() => {});
  return true;
}
...
client.on('voiceStateUpdate', (oldState, newState) => {
  ...
  if (newState.id === client.user?.id && oldState.channelId && !newState.channelId) {
    ...
    // A pinned worker should come back if it was manually disconnected.
    if (state.pinnedVoiceId) {
      const timer = setTimeout(() => reconnectPinned(guild).catch(() => {}), 1_500);
      timer.unref?.();
    }
    return;
  }
