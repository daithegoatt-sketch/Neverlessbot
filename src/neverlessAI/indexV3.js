...
const { TOOL_DEFINITIONS, createServerToolExecutor } = require('./serverToolsV2');
...
function shouldUseWeb(text) {
  ...
  if (/(لفلي|xp|تقييمي|بيلدي|build|سكيرك|skirk|genshin|قينشن|السيرفر|رساله|رسالة|روم|عضو|انجاز|achievement|رابط|invite)/iu.test(value)) return false;
...
async function handleAIMessage(message) {
  ...
  const instructions = [
    ...
    'Act as the conversational brain of Neverless, not as a menu of commands. For server-specific questions, proactively use the available Neverless tools before claiming that information is unavailable.',
    'Existing Neverless commands and systems remain authoritative and separate. Never reimplement, mutate, or bypass moderation, roles, warnings, tickets, configuration, rankings, achievements, UID linking, or Genshin scoring.',
    'The only permitted write-like action exposed to you is the safe public personal-invite tool. If the user naturally asks for the server link/invite, use it and return the resulting link instead of telling them to type #رابط. Never perform administrative commands or actions through conversation.',
    'Read-only tools may be used naturally without requiring exact command wording. Understand phrases such as كم لفلي, متى قلت كذا, شنو صار بالروم, شرايك ببيلدي, and similar conversational forms.',
    ...
    'When asked how to increase a Neverless Genshin rating, state the recorded current stats, the stated targets/deficiencies, and the recommended order of improvements. Never invent an exact future score for a hypothetical stat change; the exact score must come from rerunning the real Neverless rating system after the build changes.',
    ratingNote || '',
